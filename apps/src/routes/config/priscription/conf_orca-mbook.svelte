<script lang="ts">
	...
	$effect(() => {
		...
		if (Orca.orcaCertificate) {
			...
			//certificateInput.setAttribute('class', 'certfile');
			//certName.style.cursor = 'pointer';
			certName.classList.add('certfile');
			certName.textContent = Orca.orcaCertificate;
			...
			certName.addEventListener('click', async (event) => {
				...
				try {
					...
					certName.textContent = '証明書(.p12)を登録できます。';
					certName.style.setProperty('--after-content', '""');
					certName.setAttribute('for', 'orcaCertificate'); // ファイル選択を再度有効にする
					certName.classList.remove('certfile');
					certificateInput.classList.remove('hidden');

					// Clear the files from the input
					console.log(certificateInput.files);
...
<div class="p-4">
	...
	<div class="grid grid-cols-3 gap-4">
		<div class="flex flex-col">
			<label for="orcaCertificate" id="certName" class="font-medium text-gray-700"
				>証明書(.p12)を選択</label
			>
			<input
				type="file"
			...
			/>
			<button id="certdelete" class=" border">削除</button>
		</div>
...
<style>
	...
	#certdelete {
		@apply hidden mx-2 p-1 w-24 text-[0.8rem] text-pink-400 bg-gray-100 border border-gray-600 rounded-sm;
	}
</style>
